...
import { Observable, map, take, switchMap, of, timeout, catchError } from 'rxjs';
...
export class GameLobbyGuard implements CanActivate {
  ...
  canActivate(route: ActivatedRouteSnapshot): Observable<boolean> {
    ...
    console.log('Guard: Checking access for game lobby:', gameCode);
    ...
    return this.actions$.pipe(
      ...
      switchMap(action => {
        if (action.type === refreshGameStateFromServerSuccess.type) {
          console.log('Guard: Server refresh succeeded, checking if in lobby');
          // Check if the game is actually in lobby state
          return this.store.select(selectIsInLobby).pipe(
            take(1),
            map(isInLobby => {
              if (isInLobby) {
                console.log('Guard: Game is in lobby state, allowing access');
                return true;
              } else {
                console.log('Guard: Game is not in lobby state, redirecting to active game');
                this.router.navigate(['/game', gameCode, 'active']);
                return false;
              }
            })
          );
